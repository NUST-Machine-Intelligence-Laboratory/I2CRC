...
def get_arguments():
    parser = argparse.ArgumentParser(description='The Pytorch code of I2CRC')
...
def train(args):
    ...
    while current_epoch < total_epoch:
        ...
        for idx, dat in enumerate(train_loader):
            img_name, img, label, sal = dat
            label = label.cuda(non_blocking=True)
            sal = sal.cuda()
            
            
            logits, center_loss1, center_loss2, center_dist_loss = model(img, sal, current_epoch, label, index)
            ...
            loss = loss_val + 0.01*center_loss1 + 0.025*center_loss2 + 0.1*center_dist_loss
