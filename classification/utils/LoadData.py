...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sal_name = "set_your_dir_for_saliency_map/saliency_aug/" + img_name.strip().split('/')[-1].split('.')[0] + '.png'
        sal = cv2.imread(sal_name, 0)/255.0
        
        if self.transform is not None:
            image = self.transform(image)
            #kernel = np.ones((20, 20), np.uint8)
            #sal_dil = cv2.dilate(sal, kernel, iterations=1)
            sal = cv2.resize(sal, (256, 256), interpolation=cv2.INTER_NEAREST)
            #sal_dil = cv2.resize(sal_dil, (256, 256), interpolation=cv2.INTER_NEAREST)
            sal = torch.from_numpy(sal)
            
        if self.testing:
            #return img_name, image, self.label_list[idx], sal, sal_dil
            return img_name, image, self.label_list[idx], sal
